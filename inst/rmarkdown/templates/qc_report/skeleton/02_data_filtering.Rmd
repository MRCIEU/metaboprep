...
reason_note <- function(reason, kind = c("features", "samples")) {
  ...
  if (kind == "features") {
    return(switch(
      ...
      "user_defined_feature_missingness" = glue::glue(
        "Features exclusions based on the user defined threshold of *>={sprintf('%.0f%%', attr(metaboprep@data, 'qc_feature_missingness') * 100)}."
      ),
      "user_defined_feature_skewness" = {
        ...
        } else if (identical(skew_dir, "left")) {
          glue::glue("Feature exclusions based on user defined skewness threshold of *skew <= {-skew_thresh}*.")
  ...
  switch(
    ...
    "user_defined_sample_missingness" = glue::glue(
      "Samples exclusions based on user defined threshold of *>={sprintf('%.0f%%', attr(metaboprep@data, 'qc_sample_missingness') * 100)}."
    ),
    "user_defined_sample_totalpeakarea" = glue::glue(
      "Samples with a total-peak-area or total-sum-abundance that is *>={attr(metaboprep@data, 'qc_total_peak_area_sd')} SD from the mean."
    ),
    "user_defined_sample_pca_outlier" = {
      npcs <- attr(metaboprep@sample_summary, "qc_num_pcs_scree")
      if (is.null(npcs) || is.na(npcs)) {
        npcs <- "available informative"
      }
      glue::glue(
        "Samples that are *>={attr(metaboprep@data, 'qc_pc_outlier_sd')} SD from the mean on principal components 1-to-{npcs}."
      )
    },
    paste0(gsub("_", " ", reason), ".")
  )
...
sub_notes <- c(
  vapply(names(excl$features), reason_note, character(1), kind = "features"),
  vapply(names(excl$samples), reason_note, character(1), kind = "samples")
)
