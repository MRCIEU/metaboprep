...
```{r pca_1_2}
# get data
qcing_feature_sum <- get_feature_summary(metabolites, layer="qcing")
qcing_feature_sum <- qcing_feature_sum[, colSums(is.na(qcing_feature_sum)) < nrow(qcing_feature_sum), drop = FALSE]
pcs               <- get_pcs(metabolites, layer="qcing", drop=TRUE)
pcs               <- pcs[, grepl("^pc[0-9]+$", colnames(pcs))]
af                <- get_acceleration_factor(metabolites, layer="qcing")
pc_outlier_sd     <- metabolites@pc_outlier_sd
...
tbl1 <- data.table::data.table(
  ...
  Count = c(ncol(qcing_feature_sum),
            ...
            sum(qcing_feature_sum["independent_features_binary", ] == 1, na.rm = TRUE))
) 
