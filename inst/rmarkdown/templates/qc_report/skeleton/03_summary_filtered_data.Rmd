...
excl_samps <- unique(unname(unlist(metaboprep@exclusions$samples)))
excl_feats <- unique(unname(unlist(metaboprep@exclusions$features)))
qc_data    <- qc_data[setdiff(rownames(qc_data), excl_samps), 
                      setdiff(colnames(qc_data), excl_feats),
                      drop=FALSE]
...
```{r qc_sample_missingness, fig.width=15, fig.height=5}
missing <- rbind(
...
plots <- lapply(levels(missing$type), function(t) {
  ...
  geom_vline(data = data.frame(median = median(missing[missing$type==t, "value"])),
             aes(xintercept = median), color = "#E41A1C", linewidth = 0.75, linetype="dashed") +
  scale_color_manual(values = c(Samples = "#2171B5", Features = "#F16913", `Total abundance` = "#238B45")) +
  scale_fill_manual(values = c(Samples = "#9ECAE1", Features = "#FDD0A2", `Total abundance` = "#A1D99B")) +
  theme_classic() +
...
```{r qc_dendrogram, out.width="100%", fig.width=20, fig.height=5}
# get data
...
n <- labels(dend)
bcol = rep("#999999", length(n))
bcol[which(n %in% pv_ids )] <- "#377EB8"

# redefine elements of dendrogram
dend <- dend |>
  dendextend::set("labels", rep("", length(n))) |>
...
# plot
ggplot(dend) + 
  geom_hline(yintercept = tree_cut_height, color = "#E41A1C") +
  theme_minimal() +
  theme(
...
for (i in 1:(npcs-1)) {
  for (j in 2:npcs) {
    if (j<=i) {
      ...
      p <- ggplot(pcs, aes(x = .data[[names(pcs)[j]]], y = .data[[names(pcs)[i]]], color = as.factor(cluster_k))) +
      ...
      for (nsd in c(3, 4, 5)) {
        ...
        p <- p +
          geom_vline(xintercept = outliersV, linewidth = 0.25,
                     color = ifelse(nsd == 3, "#FFFFBF", ifelse(nsd == 4, "#FDAE61", "#D53E4F"))) +
...
w = which(excl_feats %in% colnames(qc_feature_sum))
if(length(w) > 0){
  qc_feature_sum <- qc_feature_sum[, -w, drop=FALSE]
}


# compute
count = length(  qc_feature_sum["W", ])
nacount = sum(is.na(qc_feature_sum["W", ]))
remain_count = count - nacount
normcount = sum(qc_feature_sum["W", ] >= 0.95, na.rm = TRUE)
```
...
```{r shapirowilk, fig.width=15, fig.height=4}
# get data
wdat <- rbind(
  data.frame("type" = "Raw",   "w" = qc_feature_sum["W", ]),
  data.frame("type" = "Log10", "w" = qc_feature_sum["log10_W", ])
)
wdat$type <- factor(wdat$type, levels = c("Raw", "Log10"))

LogMakesDistributionWorse <- c(
  sum(wdat$type == "Log10" & wdat$w < qc_feature_sum["W", ], na.rm = TRUE),
...
```{r w_dist_plot, fig.width=12, fig.height=5}
# Plot
...
### Estimates of skewness on raw abundances
```{r skewness_for_text}
skew_thresh <- attr(metaboprep@data, "qc_feature_skewness_threshold")
skew_dir <- attr(metaboprep@data, "qc_feature_skewness_direction")
if (is.null(skew_dir) || is.na(skew_dir)) skew_dir <- "left"

skew_df <- feature_skewness(
  data = qc_data,
  threshold = skew_thresh,
  direction = skew_dir
)

count_skew <- nrow(skew_df)
left_count <- sum(skew_df$skew < 0, na.rm = TRUE)
right_count <- sum(skew_df$skew > 0, na.rm = TRUE)
mean_skew <- signif(mean(skew_df$skew, na.rm = TRUE), digits = 3)

if (!is.null(skew_thresh) && !is.na(skew_thresh)) {
  skew_excluded_count <- sum(skew_df$exclude_by_skewness %in% TRUE, na.rm = TRUE)
  skew_threshold_text <- switch(
    skew_dir,
    "left" = glue::glue("skew <= {-skew_thresh}"),
    "right" = glue::glue("skew >= {skew_thresh}"),
    "both" = glue::glue("|skew| >= {skew_thresh}")
  )
} else {
  skew_excluded_count <- NA_integer_
  skew_threshold_text <- "No skewness-based feature exclusion threshold was applied during QC."
}
```

Across the `r count_skew` included features, `r left_count` have negative skewness and `r right_count` have positive skewness (mean skewness = `r mean_skew`). Strong negative skewness may indicate upper-end assay censoring (an upper detection bound).

`r if (!is.na(skew_excluded_count)) glue::glue("Using the configured skewness criterion ({skew_threshold_text}), {skew_excluded_count} feature(s) were excluded.") else skew_threshold_text`

```{r skew_dist_plot, fig.width=12, fig.height=5}
p <- ggplot(skew_df, aes(x = skew)) +
  geom_histogram(bins = 20, fill = "#377EB8", color = "black", alpha = 0.7) +
  geom_vline(xintercept = 0, color = "#4D4D4D", linetype = "dashed", size = 0.8) +
  theme_classic() +
  labs(x = "Feature skewness", y = "Frequency")

if (!is.null(skew_thresh) && !is.na(skew_thresh)) {
  if (identical(skew_dir, "left")) {
    p <- p + geom_vline(xintercept = -skew_thresh, color = "#E41A1C", linetype = "dashed", size = 1)
  } else if (identical(skew_dir, "right")) {
    p <- p + geom_vline(xintercept = skew_thresh, color = "#E41A1C", linetype = "dashed", size = 1)
  } else {
    p <- p + geom_vline(xintercept = c(-skew_thresh, skew_thresh), color = "#E41A1C", linetype = "dashed", size = 1)
  }
}
...
```{r outlier_summary}
# get data
# qc_data <-  metaboprep@data[, , "qc"] ## already defined on line 12
...
### Notes on outlying samples at each feature
...
qc_features    <- qc_features[!qc_features$feature_id %in% excl_feats, ]
qc_feature_sum <- t(metaboprep@feature_summary[,,"qc"]) |> as.data.frame()
qc_feature_sum$feature_id <- rownames(qc_feature_sum)
m = match(qc_features$feature_id, qc_feature_sum$feature_id)
qc_features = cbind(qc_features, missingness = qc_feature_sum[m, c("missingness")])
...
if (length(class_variables) == 0) {
  cat(" -- No feature level class variables identified or all were invariable -- \n")
} else {
  cat(paste0(" -- After filtering a total of ", length(class_variables), 
             " feature level batch variables were identified. -- \n"))
  cat(" -- They are:\n")
  cat(paste0("\t", class_variables, collapse = "\n"), "\n")
}
```
...
if (length(batch_variables) > 0 && any(qc_samples[["missingness"]] > 0, na.rm = TRUE)) {
...
ggplot(qc_sample_sum, aes(x = tpa_complete_features, y = missingness)) +
  ...
  theme_minimal() +
  labs(x = "Total abundance (complete features)", y = "Sample missingness",
       subtitle = paste0("Spearmans's cor = ", round(a$estimate, d = 4), " p-value = ",
...
```{r power_exploration, fig.width=17, fig.height=6}
p1 = continuous_power_plot( mydata = metaboprep@data[,,"qc"] )
p2 = imbalanced_power_plot( mydata = metaboprep@data[,,"qc"] )

ggpubr::ggarrange(p1, p2, labels = c("A", "B"), ncol = 2, nrow = 1, common.legend = FALSE, legend = "right")
```
