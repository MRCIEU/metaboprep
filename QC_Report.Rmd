...
knitr::include_graphics("https://raw.githubusercontent.com/MRCIEU/MetaboQC/master/images/MetaboQC_workflow.png")
```
...
```{r MissingnessMatrix, fig.width = 20, fig.height = 12}
...
ggplot(namat, aes(metabolites, individuals, fill = missingness)) + 
  geom_tile() + 
  scale_fill_gradient(low="snow", high="slategray4") +
  theme(legend.position = "none", axis.text.x = element_blank() )

```

\begin{center}
**Figure Legend:** Missingness structure across your raw data table. White cells depict missing data. Individuals are in row, metabolites are in columns. 
\end{center}
...
\begin{center}
**Figure Legend:** Raw dat -  (a) Distribution of sample missingness with sample mean illustrated by the red vertical line. (b) Distribution of feature missingness sample mean illustrated by the red vertical line. (c) Table of sample and feature missingness percentiles. A tabled version of plot a and b.  (d) Estiamtes of study samples sizes under various levels of feature missingness. 
\end{center}
...
temp = data.frame(exclustion = rownames(qcing_data$exclusion_data), count = qcing_data$exclusion_data[,1])
ggpubr::ggtexttable( temp, rows = NULL, theme = ggpubr::ttheme("mBlue"))
```

\begin{center}
**Table Legend:** Six primary quality control exclusion steps were made during the QC of the data. (1) Samples with missingness >=80% were excluded, (2) features with missingness >=80% were excluded (xenobiotics are not included in this step), (3) sample exclusions based on the user defined threshold were excluded, (4) feature exclusions based on user defined threshold were excluded (xenobiotics are not included in this step), (5) samples with a total-peak-area or total-sum-abundance that is >= N standard deviations from the mean, where N was defined by the user, were excluded, and (6) samples that are >= N standard deviations from the mean on principal component axis 1 and 2, where N was defined by the user, were excluded. 
\end{center}


### metabolite or feature reduction and principal components

A data reduction was carried out to identify a list of representitive features for generating a sample principal component analysis. This step reduces the level of inter-correlation in the data to insure that the principal components are not driven by groups of correlated features.
...
temp = data.frame( data_reduction = c("total metabolite count","metabolites inculded in data reduction","number of metabolite clusters","number of representitive metabolites"),
...
\begin{center}
**Figure Legend:** The data reduction table presents the number of metabolites at each phase of the data reduction (Spearman's correlation distance tree cutting) analysis. Principal components 1 and 2 are plotted on the right for all individuals, using the representitve features identified in the data reduction analysis. The red vertical and horizontal lines indicate the cutoffs for identifying individual outliers. The number of standard deviations from the mean to define these cuttoffs were provided by the user. 
\end{center}
...
## N 
  * Number of samples in data = `r nrow(qc_data$metabolite_data)`  
  * Number of features in data = `r ncol(qc_data$metabolite_data)`  
...
## Relative to the raw data
  ...
  * Please review details about and your log file for the number of features and samples excluded and why. 
...
dend <- as.ggdend(dend)
## plot the dendrogram
tree_plot = dend %>% ggplot() + geom_hline(yintercept = tree_cut_height, color = "coral2", width = 1)
...
screeplot = ve %>% ggplot(aes(x = PC, y = VarExp)) +
  ...
  geom_vline(xintercept = qc_data$nparallel, color = pcol[3])
...
\begin{center}
**Figure Legend:** QC'd data summary. Distributions for sample missingness, feature missingness, and total abundance of samples. Row two of the figure provides a Spearman's correlation distance clustering dendrogram highlighting the metabolites used as representative features in blue, the clustering tree cut height is denoted by the horizontal line. Row three provides a summary of the metabolite data reduction in the table, a Scree plot of the variance explained by each PC and a plot of principal component 1 and 2, as derived from the representative metabolites. The Scree plot also identifies the number of PCs estimated to be informative (vertical lines) by the Cattel's Scree Test acceleration factor (red) and Parrallel Analysis in green. Individuals in the PC plot were clustered into 4 kmeans (k) clusters, using data from PC1 and PC2. The kmeans clustering and color coding is strictly there to help provide some visualization of major axis of variation in the sample population(s).
\end{center}


## Structure among samples: top 5 PCs

A matrix plot of the top 5 principal components. Included in the plot are the 3rd (grey), 4th (orange), and 5th (red) SD from the mean, which are each possible cuttoffs for outlier filtering. 
...
### W-statistics for raw and log transformed data
...
\begin{center}
**Figure Legend:** Histogram plots of Shapiro W-statistics for raw (left) and log transformed (right) data distributions. A W-statistic value of 1 indicates the sample distribution is perfectly normal and value of 0 indicates it is perfectly uniform. Please not that log transformation of the data *may not* improve the normality of your data.
\end{center}
...
```
\begin{center}
**Table Legend:** The table reports the number of point estimates for the minimum (0%), median (50%), and maximum (100%) number of outlying features across samples and the number of outlying samples across features. 
\end{center}
...
Feature missingness may be influenced by the metabolites|features biology or pathway calssification, or your technologies methodology. The figure(s) below provides an illustrative evaluation of the proportion of *feature missigness* as a product of the variable(s) available in the raw data files. 
...
```
\begin{center}
**Table Legend:** TypeII ANOVA: the eta-squared (eta-sq) estimates are an estimation of the proportion of variation explained by each independent variable, after accounting for all other variables, as derived from the sum of squares. This is a multivariate evaluation of batch variables on *sample missingness*.
\end{center}
