...
author: "L.J.Corbin and D.A.Hughes"
date: "`r Sys.Date()`"
output: pdf_document
params: 
  Rdatafile: NA
...
## I. Description of raw, non-QC'd, `r project` `r platform` data set. 
  
## II. Data N:  
  -- Number of samples in data = `r nrow(metdata)`  
  -- Number of features in data = `r ncol(metdata)`  

## III. Power exploration for case/control and continuous outcome data:

Analytical power analysis for both case/control (50%/50%) and continuous variable analysis given you max sample size (N) and smaller N's in the instance of variabitiy in missingness among features. The case control power analysis assumes that cases and controls are split evenly in your dataset, and the N on the x-axis is the total N. 
...
## IV. Summary of Missingness:

 -- Missingness is evaluated across samples and features using the original/raw scaled data set. 

### IVa. summary table of missingness 

  -- An illustration of what your sample and feature missingness distributions look like, with their median point estimates indicated with the red vertical line. 

  -- An estimate of the proportion of missingness for quintiles of the sample and feature missingness data distribution. 
...
### IVb. your sample size at various levels of feature missingness

  -- Given your study sample size the table below provides the sample size for a features at various levels of missingness.

```{r samplesize.withmissingness, echo = FALSE, fig.width = 5, fig.height = 2.5, warning=FALSE, message=FALSE, error=FALSE}
mis[[4]][[4]]
```

### IVc. *feature missingness* as influenced by possible explanatory variables supplied by the commercial company

note: Feature missingness may be influenced by the metabolites|features biology or pathway calssification, or the mass spec methodology. 

--- The figure below provides an illustrative evaluation of the proportion of *feature missigness* as a product of the variable(s) available in the commercial raw data files. 
...
### IVd. the univariate influence of batch effects on *sample* missingness
 
--- The figure below provides an illustrative evaluation of the proportion of *sample missigness* as a product of sample batch variables provided by your supplier. 
...
### IVe. a multivariate evaluation of batch variables on *sample missingness*

TypeII ANOVA: the eta-squared (eta-sq) estimates are an estimation on the proportion of variation explained by each independent variable, after accounting for all other variables, as derived from the sum of squares. 
...
## V. Summary of Sample Total Peak Area (TPA):

Total peak area (TPA) is simply the sum of the abundances measured across all features. TPA is one measure that can be used to identify unusal samples given their entire profile. However, the level of missingness in a sample may influence TPA, so here we:  
...
### Va. corrleation between total peak area at complete features and missingness
...
### Vb. the influence of batch effects on sample total peak area
...
### Vc. a multivariate evaluation of batch variables on total peak area
...
## VI. Feature Distributions


### VIa. how many feature distributions may be considered normal distributions?
...
### VIb. W-stat distributions and does loging your data always improve the W-stat? 

-- Note that a W-statistic value of 1 indicates the sample distribution is perfectly normal and value of 0 indicates it is perfectly uniform. 
...
```
  ...
  -- do note that logging your data *may not* always improve on the normality of your data.

### VIc. All distributions printed to file
...
## V. Feature & Sample Outliers

### Va. quatile evaluation of the number of samples and features that are outliers across the data set

Below we report point estimates for the minimum (0%), median (50%), and maximum (100%) number of (1) outlying features across samples and (b) outlying samples across features. 
...
## VI. Feature correlation structure

### VIa. correlation struture among features
...
Note: A total of `r sum(table(fdata$k) == 1)` features correlated with NO other features at an Spearman's rho > 0.75, and are strictly independent. There were only `r sum(table(fdata$k) > 1)` clusters of features consisting of two or more features. The largest cluster consists of `r max(table(fdata$k))` features.


## VII. Sample correlation structure

### VIIa. correaltion structure among samples
...
1. The principle component plot above includes exclusion lines at 5SD (red), 4SD (orange), and 3SD (grey) from the mean.
...
3. Given the observations from the principle component analysis:
  + a total of `r sigpcs` have been delared significant given the estimated acceleration factor (min = 2, for any analysis).
...
## II. Relative to the raw data set:
...
## NOTE: Outlying samples at each metabolite|feature

Do not that there may be extreme outlying observations at individual metabolites|features that have not been accounted for. You may want to
